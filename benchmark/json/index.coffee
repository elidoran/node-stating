...
console.log "stating/json benchmark (#{process.pid} #{if '--turbo' in process.execArgv then '- turbo' else ''})"
ask.question 'Begin benchmark? (y/N)  ', (answer) ->
  ...
  if answer? and (answer[0] is 'y' or answer[0] is 'Y')
    ...
    require('benchmarked')({
      ...
      renameKey: (file) -> if file.stem is '_this_library' then 'stating' else file.stem
    }).run()
