...
console.log('example strings/counter-direct')

// add our two nodes
nodes.addAll({

  count: function initCount(direct) {

    var use, stop

    direct([ 'use', 'stop' ], function(nuse, nstop) {
      use = nuse
      stop = nstop
    })

    return function count(control) {
  ...
  use: function initUse(direct) {

    var count

    direct([ 'count' ], function(ncount) {
      count = ncount
    })

    return function use(control, context) {
...
}, true) // <-- means these are initializers

// ensure we start at 'count'
nodes.start('count')

// make them all direct ...
nodes.direct()
